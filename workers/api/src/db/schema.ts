...
export const clients = sqliteTable(
  'clients',
  {
    ...
    status: text('status', {
      enum: ['ACTIVE', 'READ_ONLY', 'ARCHIVED', 'DELETED'],
    })
      .notNull()
      .default('ACTIVE'),
...
export const invitations = sqliteTable(
  'invitations',
  {
    ...
    rsvpEnabled: integer('rsvp_enabled', { mode: 'boolean' })
      .notNull()
      .default(false),
  ...
  (table) => [
    uniqueIndex('idx_invitations_slug').on(table.slug),
    index('idx_invitations_published').on(table.published),
  ],
)
