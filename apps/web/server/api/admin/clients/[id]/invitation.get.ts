...
export default defineEventHandler(async (event) => {
  ...
  const row = await db
    .prepare(
      'SELECT i.*, c.wedding_date, c.wedding_tz FROM invitations i JOIN clients c ON c.id = i.client_id WHERE i.client_id = ?',
    )
    .bind(id)
    .first<{
      id: string
  ...
  if (!template.success)
    apiError('TEMPLATE_REQUIRED', 'This client still needs to choose an invitation template.', 409)
