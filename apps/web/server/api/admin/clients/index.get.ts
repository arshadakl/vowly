...
function present(row: Record<string, unknown>) {
  return {
    id: row.id, clientCode: row.client_code, name: row.name, phone: row.phone, status: row.status,
    ...
    invitation: {
      ...
      template: row.invitation_template,
      publishedAt: row.invitation_published_at ?? null,
    },
  }
}
export default defineEventHandler(async (event) => {
  ...
  const rows = await db.prepare(`SELECT c.id, c.client_code, c.name, c.phone, c.status, c.wedding_date, c.wedding_tz, c.created_at, i.id AS invitation_id, i.published AS invitation_published, i.slug AS invitation_slug, i.template AS invitation_template, i.published_at AS invitation_published_at FROM clients c LEFT JOIN invitations i ON i.client_id = c.id ${filter} ORDER BY c.created_at DESC, c.client_code DESC LIMIT ? OFFSET ?`).bind(...values, pageSize, (page - 1) * pageSize).all<Record<string, unknown>>()
