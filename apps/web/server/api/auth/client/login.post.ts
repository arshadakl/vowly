...
export default defineEventHandler(async (event) => {
  ...
  const client = await getEnv(event).DB.prepare('SELECT id, client_code, name, phone, status FROM clients WHERE passcode = ? AND phone = ?').bind(parsed.data.passcode, parsed.data.phone).first<{ id: string; client_code: string; name: string; status: string }>()
