...
async function upload(field: 'coverImage' | 'brideImage' | 'groomImage', event: Event) {
  ...
  if (!file) return
  try {
    const upload = await api<{ uploadUrl: string; key: string }>('/client/uploads', { method: 'POST', body: { field, contentType: file.type } })
    await api(upload.uploadUrl.replace('/api', ''), { method: 'POST', body: file, headers: { 'Content-Type': file.type } })
    draft.value[field] = upload.key
    notice.value = 'Image uploaded. Save the invitation to keep it.'
  } catch (error: unknown) { errorMessage.value = error instanceof Error ? error.message : 'Image upload failed.' }
}
