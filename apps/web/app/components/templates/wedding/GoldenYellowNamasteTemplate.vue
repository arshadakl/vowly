...
const def = getTemplateDefinition('golden-yellow-namaste')
...
const heroEventText = computed(() => inv.value.customization.text.heroEventText || 'with the blessings of their families invite you')
const countdownTitle = computed(() => inv.value.customization.text.countdownTitle || 'The Auspicious Muhurtham')
  ...
  >
    ...
    <section id="hero-section" class="relative w-full overflow-hidden">
      <div class="absolute inset-0 z-0">
        <img
          ...
          loading="eager"
        >
        <div class="absolute inset-0 bg-gradient-to-b from-white/5 via-transparent to-[#fdf8ef]" />
      </div>

      <div class="relative z-10 mx-auto flex min-h-[94vh] max-w-lg flex-col items-center justify-end px-5 pb-40 pt-28 text-center">
        <!-- Decorative star ornament -->
        <div class="mb-5 flex items-center gap-2">
          <div class="h-px w-8 bg-amber-400/70" />
          <span class="text-base text-amber-600">✦</span>
          <div class="h-px w-8 bg-amber-400/70" />
        </div>

        <!-- Groom -->
        <h1 class="text-[clamp(1.85rem,9.5cqw,2.5rem)] font-medium tracking-wide text-stone-800 break-words leading-[1.1]">
        ...
        <!-- Divider -->
        <div class="my-4 flex items-center gap-2 text-amber-600">
        ...
        <div class="mt-5">
          <p class="max-w-[270px] text-[13.5px] font-light leading-relaxed text-stone-600">
            <TemplateEditable field="heroEventText" :value="heroEventText" as="span" placeholder="with the blessings of their families invite you" />
          </p>
        </div>

        <!-- Date & Time -->
        <div class="mt-7 flex flex-col items-center gap-2.5 sm:flex-row sm:gap-4">
          <div class="flex items-center gap-2 rounded-full border border-amber-300/70 bg-white/75 px-4 py-1.5 backdrop-blur-sm">
            <Calendar :size="13" class="text-amber-700" />
    ...
    <div class="relative z-10 mx-auto max-w-lg px-5 pb-20">
      ...
      <section class="mt-4 text-center">
        <h2 class="mb-7 text-[11px] font-medium uppercase tracking-[0.28em] text-amber-800">
          <TemplateEditable field="countdownTitle" :value="countdownTitle" as="span" placeholder="The Auspicious Muhurtham" />
        </h2>
        <TemplateCountdown :date="weddingDate" :time-zone="weddingTz" />
      </section>

      <!-- Venue -->
      <section class="mt-12 rounded-3xl border border-amber-200/70 bg-white p-6 shadow-sm">
        <div class="mb-4 flex items-center justify-center gap-2">
          <MapPin :size="15" class="text-amber-700" />
          <span class="text-[10px] font-medium uppercase tracking-[0.25em] text-amber-800">
            Venue
          </span>
        </div>
        ...
        <a
          ...
          class="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-amber-700 py-2.5 text-[13px] font-medium tracking-wide text-white transition hover:bg-amber-800 active:scale-[0.98]"
        >
      ...
      <footer class="mt-14 flex flex-col items-center text-center">
        <div class="mb-4 flex h-[52px] w-[52px] items-center justify-center rounded-full border border-amber-300/70 bg-white text-sm font-medium tracking-widest text-amber-900 shadow-sm">
          {{ monogram }}
        </div>
        <p class="max-w-[240px] text-[12.5px] font-light leading-relaxed text-stone-500">
          We look forward to celebrating this sacred and joyous occasion with you.
        </p>
        <div class="mt-5 flex items-center gap-2 text-amber-500">
          <span class="text-sm">✦</span>
